...
module.exports = {
  ...
  get_distribution: function(richlist, stats, cb) {
    var distribution = {
      supply: stats.supply,
      t_1: {percent: 0, total: 0 },
      t_2_30: {percent: 0, total: 0 },
      t_31_68: {percent: 0, total: 0 },
      t_69_100: {percent: 0, total: 0 },
    ...
    lib.syncLoop(richlist.balance.length, function (loop) {
      ...
      if (count = 1 ) {
        distribution.t_1.percent = distribution.t_1.percent + percentage;
        distribution.t_1.total = distribution.t_1.total + (richlist.balance[i].balance / 100000000);
      }

      if (count <= 30 && count > 2) {
        distribution.t_2_30.percent = distribution.t_2_30.percent + percentage;
        distribution.t_2_30.total = distribution.t_2_30.total + (richlist.balance[i].balance / 100000000);
      }

      if (count <= 68 && count > 31) {
        distribution.t_31_68.percent = distribution.t_31_68.percent + percentage;
        distribution.t_51_75.total = distribution.t_51_75.total + (richlist.balance[i].balance / 100000000);
      }

      if (count <= 100 && count > 69) {
        distribution.t_69_100.percent = distribution.t_69_100.percent + percentage;
        distribution.t_69_100.total = distribution.t_69_100.total + (richlist.balance[i].balance / 100000000);
      }

      loop.next();
    }, function() {
      distribution.t_101plus.percent = parseFloat(100 - distribution.t_69_100.percent - distribution.t_31_68.percent - distribution.t_2_30.percent - distribution.t_1.percent - (settings.richlist_page.burned_coins.include_burned_coins_in_distribution == true && richlist.burned > 0 ? ((richlist.burned / 100000000) / stats.supply) * 100 : 0)).toFixed(2);
      distribution.t_101plus.total = parseFloat(distribution.supply - distribution.t_69_100.total - distribution.t_31_68.total - distribution.t_2_30.total - distribution.t_1.total - (settings.richlist_page.burned_coins.include_burned_coins_in_distribution == true && richlist.burned > 0 ? (richlist.burned / 100000000) : 0)).toFixed(8);
      distribution.t_1.percent = parseFloat(distribution.t_1.percent).toFixed(2);
      distribution.t_1.total = parseFloat(distribution.t_1.total).toFixed(8);
      distribution.t_2_30.percent = parseFloat(distribution.t_2_30.percent).toFixed(2);
      distribution.t_2_30.total = parseFloat(distribution.t_2_30.total).toFixed(8);
      distribution.t_31_68.percent = parseFloat(distribution.t_31_68.percent).toFixed(2);
      distribution.t_31_68.total = parseFloat(distribution.t_31_68.total).toFixed(8);
      distribution.t_69_100.percent = parseFloat(distribution.t_69_100.percent).toFixed(2);
      distribution.t_69_100.total = parseFloat(distribution.t_69_100.total).toFixed(8);
